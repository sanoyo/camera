...
class NFOV():
    ...
    def _get_screen_img(self):
        # np.linspace(0, 1, self.width) → 0～1を800等分する
        # np.linspace(0, 1, self.height) → 0～1を400等分する
        # meshgrid　→　上2つをあわせる
    ...
    def saveImg(self, nfov, img, center_point2):
        test = nfov.toNFOV(img, center_point2)
        nfov = cv2.cvtColor(test, cv2.COLOR_BGR2RGB)
        cv2.imwrite('test.jpg', nfov)

    def saveImg2(self, nfov, img, center_point2):
        test = nfov.toNFOV(img, center_point2)
        nfov = cv2.cvtColor(test, cv2.COLOR_BGR2RGB)
        cv2.imwrite('test2.jpg', nfov)

    def saveImg3(self, nfov, img, center_point2):
        test = nfov.toNFOV(img, center_point2)
        nfov = cv2.cvtColor(test, cv2.COLOR_BGR2RGB)
        cv2.imwrite('test3.jpg', nfov)

# test the class
if __name__ == '__main__':
    import imageio as im
    img = im.imread('images/360.png')
    nfov = NFOV()
    # 0.1(画面左)　～ 1.0(画面右)を取得
    center_point = np.array([0.15, .5])
    center_point2 = np.array([0.5, .5])
    center_point3 = np.array([0.85, .5])  # camera center point (valid range [0,1])

    # 画像を3枚保存
    nfov.saveImg(nfov, img, center_point)
    nfov.saveImg2(nfov, img, center_point2)
    nfov.saveImg3(nfov, img, center_point3)
